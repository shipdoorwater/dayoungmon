...
- 🎯 **분석 모드 선택**: 빠른 키워드 검사 vs AI 정밀 분석
- 💰 **비용 관리**: 일일/월간 사용량 제한 및 실시간 모니터링
- 🖥️ **직관적인 GUI**: 사용하기 쉬운 그래픽 인터페이스
- 💾 **결과 저장**: 검토 결과를 텍스트 파일로 내보내기

### 계획된 기능 (Phase 3)
- 🏠 **로컬 AI**: Ollama를 사용한 완전 오프라인 AI 분석
- 📈 **하이브리드 모드**: 여러 분석 모드의 결과 비교 및 통합
...
python test_ai_features.py  # AI 기능 테스트
```
...
2. **분석 모드 선택**: 
   ...
   - **AI 정밀 분석**: Claude API를 통한 맥락적 분석 (API 키 필요)
...
│   │   └── ai_analyzer.py         # AI 기반 법규 분석
│   ├── ui/
│   │   └── main_window.py         # GUI 인터페이스 (AI 모드 지원)
│   └── data/
├── assets/
...
├── test_ai_features.py            # AI 기능 테스트
├── requirements.txt               # 의존성 패키지 (AI 라이브러리 포함)
